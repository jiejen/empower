...
function CreateReport() {
  ...
  const handleSubmit = () => {
    ...
    const selectedApplianceData = appliances.filter(app => selectedAppliances.includes(app.id));

    const appliancesWithoutData = selectedApplianceData.filter(app => !applianceHasDataForDateRange(app, startDate, endDate));
      ...
      <div style={{padding: '32px', maxWidth: '1000px'}}>
        <div style={{backgroundColor: 'white', borderRadius: '8px', border: '1px solid #e5e7eb', padding: '32px'}}>
          ...
          <div style={{marginBottom: '24px'}}>
            ...
            <input type="text" value={reportName} onChange={(e) => setReportName(e.target.value)} placeholder="e.g., Monthly Kitchen Usage" style={{width: '100%', padding: '10px 12px', border: '1px solid #d1d5db', borderRadius: '6px', fontSize: '16px', outline: 'none', boxSizing: 'border-box'}}
            />
          </div>
          ...
          <div style={{marginBottom: '24px'}}>
            ...
            {dateError && (
              <div style={{padding: '10px 12px', marginBottom: '12px', backgroundColor: '#fee2e2', color: '#991b1b', borderRadius: '6px', fontSize: '13px', fontWeight: '500'}}>
                {dateError}
              </div>
            )}
          ...
          <div style={{marginBottom: '24px'}}>
            ...
            {applianceValidationError && (
              <div style={{padding: '10px 12px', marginBottom: '12px', backgroundColor: '#fef3c7', color: '#92400e', borderRadius: '6px', fontSize: '13px', fontWeight: '500'}}>
                {applianceValidationError}
              </div>
            )}
            {startDate && endDate && (
            ...
            <div style={{border: '1px solid #d1d5db', borderRadius: '6px', padding: '12px', maxHeight: '240px', overflowY: 'auto', backgroundColor: '#f9fafb'}}>
              {loading ? (
                ...
                <div style={{textAlign: 'center', padding: '20px', color: '#6b7280'}}>
                  No appliances available. Please add appliances first. <br/><br/>
                    <button onClick={() => navigate('/add-appliance')} style={{padding: '10px 20px', backgroundColor: '#28a745', color: 'white', border: 'none', borderRadius: '6px', fontSize: '14px', fontWeight: '500', cursor: 'pointer', transition: 'background-color 0.2s'}} onMouseEnter={(e) => e.target.style.backgroundColor = '#218838'} onMouseLeave={(e) => e.target.style.backgroundColor = '#28a745'}>
                        + Add Appliance
                    </button>
                </div>
              ) : (
                    ...
                    <label key={appliance.id} style={{display: 'flex', alignItems: 'center', padding: '12px', cursor: isDisabled ? 'not-allowed' : 'pointer', borderRadius: '6px', marginBottom: '8px', transition: 'all 0.2s', backgroundColor: isDisabled ? '#f3f4f6' : isSelected ? '#dbeafe' : 'white', border: `1px solid ${isDisabled ? '#e5e7eb' : isSelected ? '#3b82f6' : '#e5e7eb'}`, opacity: isDisabled ? 0.6 : 1}} title={isDisabled ? 'This appliance does not have data for the selected date range' : ''}>
                      ...
                      <div style={{flex: 1}}>
                        <div style={{fontSize: '15px', fontWeight: '500', color: isDisabled ? '#9ca3af' : '#1f2937', marginBottom: '2px'}}>
                          {appliance.name}
                          {isDisabled && (
                            <span style={{marginLeft: '8px', fontSize: '12px', color: '#dc2626', fontWeight: '400'}}>
